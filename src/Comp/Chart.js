    ...
    if (dataType === "marketCap") {
      return (
        <div className="custom-tooltip bg-white p-4 border border-gray-300 rounded shadow">
          ...
          <p className="intro">{`Market Cap: $${(payload[0].value / 1e9).toFixed(2)}B`}</p>
          <p className="desc">{`Vol 24h: $${(payload[0].payload.volume / 1e9).toFixed(2)}B`}</p>
        </div>
      );
    } else {
      return (
...
const Chart = ({ coingeckoId }) => {
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        console.log(`Fetching data for ${coingeckoId} with timeframe ${timeframe} and dataType ${dataType}`);
        ...
        if (timeframe === "max" || timeframe === "365") {
          formattedData = response.data[dataTypes[dataType]].map((item, index) => ({
    ...
    <div className="space-y-2 -mt-12">
      <div className="flex flex-col items-end space-y-1">
        ...
        <div className="flex space-x-2">
          <button
            className={`px-4 py-1 ${dataType === "price" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800"}`}
            onClick={() => setDataType("price")}
          >
            Price
          </button>
          <button
            className={`px-4 py-1 ${dataType === "marketCap" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800"}`}
        ...
        <div className="flex space-x-1">
          {Object.keys(timeframes).map((key) => (
            <button
              key={key}
              className={`px-3 py-1 ${timeframe === timeframes[key] ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800"}`}
